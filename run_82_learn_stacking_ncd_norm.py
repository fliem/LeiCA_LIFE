...
target = 'age'
selection_crit_list = ['bothSexes_neuH_FD06_ncd_norm']
...
for training in ['training_life_only']:
    print(training)
    root_path_template = os.path.join(in_data_path,'learning_out_ncd_norm_' + date_str, training,'pdfs/single_source_model_reg_{reg}_predicted')
    out_path_template =  os.path.join(in_data_path, 'learning_out_ncd_norm_' + date_str, training,'stacking/stacking_out_reg_{reg}')
